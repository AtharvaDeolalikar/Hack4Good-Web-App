          ...
          <Box sx={{ bgcolor: "#162534" }}>
            ...
            <DialogContent>
              <DialogContentText>
                ...
                <Stack
                  direction="row"
                  sx={{ flexWrap: "wrap", gap: 1, mt: 1, mb: 2 }}
                >
                ...
                <Stack spacing={1}>
                  <Link
                    ...
                    href={team.submission.projectLinks.githubRepo}
                    target="_blank"
                  >
                    {team.submission.projectLinks.githubRepo}
                  </Link>
                  <Link
                    sx={{ wordWrap: "break-word" }}
                    href={team.submission.projectLinks.videoDemo}
                    target="_blank"
                  >
                    {team.submission.projectLinks.videoDemo}
                  </Link>
                  {team.submission.projectLinks.deployed && (
                    <Link
                      sx={{ wordWrap: "break-word" }}
                      href={team.submission.projectLinks.deployed}
                      target="_blank"
                    >
                      {team.submission.projectLinks.deployed}
                    </Link>
              ...
              <Button
                sx={{ float: "right" }}
                onClick={() =>
                  setSubmissionDialog({ ...submissionDialog, show: false })
                }
              >
                Close
              </Button>
            ...
            <Box sx={{ display: "flex" }}>
              ...
              >
                ...
                <Grid item md={2} sx={{ textAlign: "center" }}>
                  <Chip color="info" label="Submission" />
                </Grid>
              </Grid>
            </Box>
            <TransitionGroup>
              {teams
                ...
                .filter((item) => {
                  if (filter === "all") {
                    ...
                    filter === true &&
                    item.submission.submitted === true
                  ) {
                    return item;
                  } else if (
                    filter === false &&
                    item.submission.submitted === false
                  ) {
                    return item;
                  }
                })
                .map((team) => {
                  return (
                    <Collapse in={true} key={team.index}>
                      <Box sx={{ display: "flex" }}>
                        <Grid
                          container
                          columnSpacing={3}
