...
function Submission() {
  ...
  const [technologies, setTechnologies] = useState(
    (isSubmitted && submission.technologiesUsed) || []
  );
  ...
  useEffect(() => {
    var deadline = context.publicSettings.submissionDeadline.toDate().getTime();
  ...
  async function MakeSubmission(e) {
    ...
    if (
      !e.target.problemStatementID.value.startsWith("P") &&
      !e.target.problemStatementID.value.startsWith("OB")
    ) {
    ...
    for (const item in submissionData) {
      if (
        typeof submissionData[item] == "string" &&
        submissionData[item].length === 0
      ) {
    ...
    if (
      submissionData.projectLinks.githubRepo.length === 0 ||
      submissionData.projectLinks.videoDemo.length === 0 ||
      submissionData.projectLinks.dataset.length === 0
    ) {
      context.showAlert(
        "warning",
        `You must provide the required project link(s)`
      );
      setLoadButton(false);
      return false;
    }
    ...
    for (const link in submissionData.projectLinks) {
      if (
        submissionData.projectLinks[link] !== "" &&
        !submissionData.projectLinks[link].startsWith("https")
      ) {
    ...
    <>
      ...
      >
        ...
        <Grid
          ...
          mt={1}
          mb={1}
          sx={{ p: 4, borderRadius: 2, bgcolor: "#162534" }}
        >
          <Stack spacing={2}>
        ...
        >
          <Stack spacing={2}>
            ...
            <TextField
              ...
              fullWidth
              defaultValue={
                isSubmitted ? submission.projectLinks.githubRepo : ""
              }
            ></TextField>
            <TextField
              name="videoDemo"
              ...
              disabled={!(!timer.expired && editable)}
              defaultValue={
                isSubmitted ? submission.projectLinks.videoDemo : ""
              }
              fullWidth
            ></TextField>
            <TextField
